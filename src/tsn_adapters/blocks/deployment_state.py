...
class DeploymentStateModel(DataFrameModel):
    ...
    deployment_timestamp: Series[pd.DatetimeTZDtype] = Field(nullable=True, dtype_kwargs={"tz": "UTC", "unit": "ns"})
...
class S3DeploymentStateBlock(DeploymentStateBlock):
    ...
    def _create_empty_df(self) -> pd.DataFrame:
        ...
        """
        return pd.DataFrame(
            {"stream_id": pd.Series(dtype="str"), "deployment_timestamp": pd.Series(dtype="datetime64[ns, UTC]")}
        )
