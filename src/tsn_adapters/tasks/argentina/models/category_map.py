...
class SepaProductCategoryMapModel(DataFrameModel):
    ...
    def from_url(cls: type[U], url: str, sep: str = "|", compression: CompressionOptions = None) -> DataFrame[U]:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                raise ValueError(f"Failed to download category map from {url}: {e!s}") from e
            ...
            try:
                ...
                raise ValueError(f"Failed to save category map file: {e!s}") from e

            # Read the CSV from the zip file
            try:
                df = pd.read_csv(temp_file, compression=compression, sep=sep)
            except Exception as e:
                raise ValueError(f"Failed to read category map as CSV: {e!s}") from e
    ...
    def task_from_url(
        url: str, sep: str = "|", compression: CompressionOptions = None
    ) -> DataFrame["SepaProductCategoryMapModel"]:
