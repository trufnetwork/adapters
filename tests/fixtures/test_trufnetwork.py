...
TSN_DB_CONTAINER = ContainerSpec(
    ...
    args=[
        ...
        "--chain.consensus.timeout-propose", "100ms",
        "--chain.consensus.timeout-prevote", "100ms",
        "--chain.consensus.timeout-precommit", "100ms",
        "--chain.consensus.timeout-commit", "100ms"
    ],
    env_vars=[
...
def wait_for_postgres_health(max_attempts: int = 30) -> bool:
    ...
    for i in range(max_attempts):
        try:
            ...
            logger.error(f"Error checking postgres health: {str(e)}")
        time.sleep(1)
    return False
...
def wait_for_tsn_health(max_attempts: int = 10) -> bool:
    ...
    for i in range(max_attempts):
        try:
            ...
            logger.debug(f"Error checking TSN-DB health (attempt {i+1}/{max_attempts}): {str(e)}")
        time.sleep(1)
    return False
...
def start_container(spec: ContainerSpec, network: str) -> bool:
    ...
    try:
        result = run_docker_command(args, check=True)
...
def stop_container(name: str) -> bool:
    ...
    try:
        result = run_docker_command(["stop", name], check=True)
...
@pytest.mark.skipif(
    ...
    reason="Local development fixture tests, skipped in CI"
)
